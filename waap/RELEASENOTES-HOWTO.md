...
When the website for a new Aero-WAAP release is published, the release notes must be prepared manually. The file
that needs to be updated is this one:

----
    src/docs/aero-waap-releasenotes.md
----

The new information is always inserted at the top of the file, right after the main
title "Aero WAAP Release Notes".
...
- Then add the changelog entries from the various changelog of the Aero WAAP proxy
...
- In each release entry category (such as "New features and improvements) add all the
  changelogs from the Aero WAAP proxy first, e.g. support for a new filter technology
