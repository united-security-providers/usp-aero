...
Open the listener and go to [Timeouts & Limits](../reference/gui/listeners/timeouts-and-limits). Set Maximum Concurrent
Connections and Maximum Concurrent HTTP/2 Streams to match the traffic you expect, and check the
Connection Buffer Limit and the timeout values for connections that never complete or never end
(Connection Acceptance Timeout, TLS Handshake Timeout, Request Headers Received Timeout, Request
Total Received Timeout, Stream Idle Timeout, Connection Idle Timeout, Connection Maximum Duration).
...
- **Slowloris (slow header attack)** - many connections are opened and their request headers are sent
  ...
  Received Timeout closes a connection whose headers do not arrive in time; Maximum Concurrent
  Connections caps how many slots the attack can hold in total.
...
- **TLS handshake stalling** - connections are accepted but the handshake is never finished, each one
  holding a socket and handshake state. Connection Acceptance Timeout and TLS Handshake Timeout
  bound both phases.
